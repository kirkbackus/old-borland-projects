...
class BaseSocket {
    ...
    unsigned short ReadUShort() { readushort(0); }
    int ReadInt() { readint(0); }
    unsigned int ReadUInt() { readuint(0); }
    float ReadFloat() { readfloat(0); }
    double ReadDouble() { readdouble(0); }
    char* ReadString() { readstring(0); }
    void ReadStruct(char* buffer, int size) { buffer = readchars(size,0); }
