...
class Textures {
  ...
  GLuint txt[100];  //Space for textures
  int id;
  int numtex;
  HINSTANCE hi;
  ...
  int NumTextures() {return(numtex);}

  bool AddTexture128(const char* fname,int index, int transparent);
  bool AddTexture64(const char* fname,int index);
};

bool Textures::AddTexture128(const char* fname, int index, int transparent) {
  HBITMAP image;
  image=(HBITMAP)LoadImageA(hi,fname,IMAGE_BITMAP,0,0,LR_LOADFROMFILE);
  ...
  for (int x=0;x<128;x++) {
    for (int y=0;y<128;y++) {
      if (transcolor!=RGB(bits[x][y][0],bits[x][y][1],bits[x][y][2]) || !transparent)
        bits[x][y][3]=255;
    }
  }
